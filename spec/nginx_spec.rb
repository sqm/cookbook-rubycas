...
describe 'rubycas::nginx' do
  ...
  it "should include the logrotate recipe" do
    expect(chef_run_converge).to include_recipe 'logrotate'
  end
  ...
  it "should have a logrotate configuration to rotate the ngix logs" do
    expect(chef_run_converge).to render_file('/etc/logrotate.d/nginx')
  end

   it "should create the nginx logrotate configuration file with the correct content" do
logrotate_nginx = <<EOF
...
EOF
    expect(chef_run_converge).to render_file("/etc/logrotate.d/nginx").with_content(logrotate_nginx)
  end
end
